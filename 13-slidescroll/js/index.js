...
//Generic debounce function to prevent CPU overload
...
function checkSlide(e) {
	sliderImages.forEach(slideImg => {
		...
		const slideInAt =
			window.scrollY + window.innerHeight - (slideImg.height / 10);
		...
		if (isVisible && isNotScrolledPast) slideImg.classList.add('active');
